...
- **Never** add Electron, Tauri, or any desktop wrapper.
- **Never** perform physics calculations, numerical integration, or control logic in frontend code.
- **Never** use Pydantic v1 API. This project uses Pydantic v2 exclusively.
- **Never** use `if/elif` chains for `ControlMode` dispatch; use `match/case`.
- **Never** import simulation or controller logic into `websocket_manager.py`.
- **Never** add a charting library (ECharts, Plotly, Chart.js, Recharts). All charts use Canvas 2D.
- **Always** clamp torque output to `[-max_motor_torque, max_motor_torque]` in any new controller.
...
- Physics rate and telemetry rate are decoupled via `BroadcastThrottle`; do not broadcast every physics step.